...
describe("DesktopBackendConfiguration", () => {
  ...
  it.effect("resolveWsl preserves existing WSLENV entries when forwarding backend secrets", () =>
    Effect.gen(function* () {
      ...
      const previousWorkflowCatalogReadToken = process.env.T3_WORKFLOW_CATALOG_READ_TOKEN;
      ...
      try {
        ...
        process.env.T3_WORKFLOW_CATALOG_READ_TOKEN = "catalog-read-token";
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.env.T3_WORKFLOW_CATALOG_READ_TOKEN, "catalog-read-token");
          ...
          assert.equal(
            config.env.WSLENV,
            "GOPATH/p:OPENAI_API_KEY/u:EMPTY::AZURE_DEVOPS_EXT_PAT/u:ANTHROPIC_API_KEY:T3_WORKFLOW_CATALOG_READ_TOKEN:T3CODE_OTLP_HEADERS:T3CODE_OTLP_PROTOCOL",
          );
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
        ...
        restoreEnv("T3_WORKFLOW_CATALOG_READ_TOKEN", previousWorkflowCatalogReadToken);
