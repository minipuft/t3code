...
describe("combined panel persistence v14", () => {
  it.each([12, 13])(
    "preserves both supported dialects and the active singleton from v%s",
    (version) => {
      const surfaces = [
        { id: "plan", kind: "plan" },
        { id: "actions", kind: "actions" },
        ...
        { id: "device", kind: "device" },
        {
          id: "device:host:sim",
          kind: "device",
          target: { hostId: "host", deviceId: "sim", platform: "ios", name: "Phone" },
        },
      ];
      const migrated = migratePersistedRightPanelState(
        {
          byThreadKey: {
            "env:thread": {
              ...
              surfaces: [...surfaces, { id: "unknown", kind: "unknown" }],
            },
          },
        },
        version,
      );
      expect(migrated.byThreadKey["env:thread"]).toEqual({
        isOpen: true,
        activeSurfaceId: "skills",
        surfaces,
      });
  ...
  it("does not revive the pre-v9 inline-plan predecessor", () => {
    const migrated = migratePersistedRightPanelState(
      {
        byThreadKey: {
          "env:thread": {
