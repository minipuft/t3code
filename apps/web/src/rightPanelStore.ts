...
// v14 combines upstream device/linked-PR and contextual Workbench singleton dialects.
            ...
            .map(([threadKey, threadState]) => {
              ...
              const surfaces = Array.isArray(validThreadState?.surfaces)
                ? validThreadState.surfaces.flatMap<RightPanelSurface>((surface) => {
                    ...
                    if (candidate.kind === "plan" && version < 9) return [];
                    if (
                      candidate.kind === "pull-requests" ||
                      candidate.kind === "diff" ||
                    ...
                    if (surface.kind === "device") {
                      if (surface.id === "device" && surface.target === undefined) return [surface];
                      ...
                      return target &&
                        ...
                        surface.id ===
                          `device:${encodeURIComponent(target.hostId)}:${encodeURIComponent(target.deviceId)}`
                        ? [surface]
                        : [];
                    }
