...
## Workflow catalog authority

Catalog lists remain metadata-only. Executable prompt details are read on demand through the
environment’s authenticated catalog connection; source credentials stay on the server. Skills
remain provider-owned invocations. The [Workbench adapter](./agent-workbench-adapter.md) preserves
this authority boundary across local and remote clients.
