...
describe("ServerSettings workflow catalog source", () => {
  it("defaults to disabled and accepts an explicit HTTP source", () => {
    expect(decodeServerSettings({}).workflowCatalogSource).toBeNull();
    expect(
      decodeServerSettings({
        workflowCatalogSource: { kind: "http", baseUrl: "http://127.0.0.1:4317" },
      }).workflowCatalogSource,
    ).toEqual({ kind: "http", baseUrl: "http://127.0.0.1:4317" });
  });

  it("accepts source replacement and disable patches", () => {
    expect(
      decodeServerSettingsPatch({
        workflowCatalogSource: { kind: "http", baseUrl: "https://catalog.example.test" },
      }).workflowCatalogSource,
    ).toEqual({ kind: "http", baseUrl: "https://catalog.example.test" });
    expect(
      decodeServerSettingsPatch({ workflowCatalogSource: null }).workflowCatalogSource,
    ).toBeNull();
  });
});
